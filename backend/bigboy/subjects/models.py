...
from bigboy.subjects.prompts import generate_bites
...
class Topic(models.Model):
    ...
    def generate_bites(self):
        ...
        to_create = []
        for i, item in enumerate(response.bites):
            name = (getattr(item, 'title', None) or '').strip() or f'Part {i + 1}'
            text = (getattr(item, 'content', None) or '').strip()
            if not text:
                continue
            to_create.append(
                Bite(topic=self, name=name[:100], bite=text),
            )
        if not to_create:
            ...
            if body:
                to_create.append(
                    Bite(
                        topic=self,
                        name=(body.split('\n', 1)[0].strip()[:100] or 'Overview'),
                        bite=body[:8000],
                    ),
                )
        Bite.objects.bulk_create(to_create)
        return len(to_create)

    def get_total_number_of_bites_by_topic(self):
        return self.topic_bites.count()

    def regenerate_bites(self):
        """Remove non-locked bites and generate fresh instructional bites from ``content``."""
        self.topic_bites.filter(is_locked=False).delete()
        return self.generate_bites()
...
class Bite(models.Model):
    ...
    is_locked = models.BooleanField(
        default=False,
        help_text='When true, preserved during topic bite regeneration (human-in-the-loop).',
    )
